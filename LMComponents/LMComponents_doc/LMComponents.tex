...
\hyperref[lmfilters.TDigFilter]{TDigFilter}. Ancestor class for all digital filters.\\
\hyperref[lmfilters.TOneFreqFilter]{TOneFreqFilter}. Ancestor class for all filters with a single corner frequency.\\
\hyperref[lmfilters.TFIRFilter]{TFIRFilter}. Ancestor class for filters with finite impulse responce filters.\\
\hyperref[lmfilters.TMovAvFilter]{TMovAvFilter}. Moving average filter\\
\hyperref[lmfilters.TGaussFilter]{TGaussFilter}. Gaussian filter. The only one, which can be used only for filtering ready data, but not real time.\\
\hyperref[lmfilters.TMedianFilter]{TMedianFilter}. Median filter.\\

Most of them can be used both for filtering of earlier sampled data and for filtering in real time, during data acquisition. To make the components format-independent, data points are fed into filter with user-defined \code{OnInput} event (see \ref{lmfilters-TInputFunc} for type description and \ref{lmfilters.TDigFilter-OnInput} for simplest example of implementation) and returned by another user-defined \code{OnOutput} event (\ref{lmfilters-TOutputproc} and \ref{lmfilters.TDigFilter-OnOutput}). You can drop a component onto a form and define \code{OnInput} and \code{OnOutput} events. Afterwards, set sampling rate and cut-off frequency by call of \code{SetupFilter} (\ref{lmfilters.TOneFreqFilter-SetupFilter}) method. For moving average filter (\ref{lmfilters.TMovAvFilter}), one has an alternative to define averaging window length instead of defining sampling rate and cut-off frequency. For median filter (\ref{lmfilters.TMedianFilter}), this is the only possibility, because for this filter cut-off frequency is not defined. After setting up the filter properties,  either call \code{Filter} (\ref{lmfilters.TDigFilter-Filter}) method for off-line filtering of existing data, or initiate filtering in real time. To do so, call \code{InitFiltering}  (\ref{lmfilters.TDigFilter-InitFiltering}) once, and then call \code{NextPoint} (\ref{lmfilters.TDigFilter-NextPoint}) method whenever new datapoint is aquired. Method \code{Filter} calls \code{OnInput} event when needs a next input value, and \code{OnOutput} when it is ready to return a next output value. Similarly, \code{NextPoint} calls \code{OnInput} and \code{OnOutput} when is invoked. This technique with \code{OnInput} and \code{OnOutput} events makes the components independent of a format of data which are filtered. 
...
\begin{itemize}\item[\declarationitem{Declaration}\hfill]
	\begin{flushleft}
		\code{
			procedure NextPoint; virtual; abstract;}
	\end{flushleft}
	\item[\descriptiontitle{Description}] Descendants use it for real-time filtering. Calls \code{OnInput} (\ref{lmfilters.TDigFilter-OnInput}) and passes \code{Index} (\ref{lmfilters.TDigFilter-index}) value as parameter to get input value, after that calculates filtered output value and calls \code{onOutput} (\ref{lmfilters.TDigFilter-OnOutput}), to which passes \code{Index} and calculated value as parameters and increments \code{Index}.
	\item[\descriptiontitle{How to use.}] Define \code{OnInput} and \code{OnOutput} events such that \code{OnInput} returns a newly acquired data value and \code{OnOutput} returns in \code{Val} the calculated filtered value to your final (filtered) dataset. Before the beginning of data aquisition, call \code{InitFiltering}. Value of \code{Index} field is initially set to zero. During the aquisition cycle, simply call \code{NextPoint} whenever new data value is acquired. 
\end{itemize}
...
\paragraph{SetXGridDist, SetYGridDist}\hspace*{\fill}
